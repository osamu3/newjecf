...
$(function () {
	...
	socket.on('S2C:sendAllEventData', function (allEvntData) {
		...
		for (let itm of allEvntData) {//イベントデータを一つづつ取り出す。 itmはJSonStrings
			...
			//①全ての事象に登録
			$('#ulListEvnt-0').append('<a><li class="evntList" id="' + itm.eventId + '" onClick=eventListClick(this)>' + linkStr + '</li></a>');
			//②カテゴリー落下物に登録
